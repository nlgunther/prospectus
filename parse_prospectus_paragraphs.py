...
class LineTests(object):
    ...
    def test(self, lineobj:dict, func:Callable = all) -> list[bool]:
        '''
        Use func = lambda arg: None to get list of individual test results
        '''
...
pagenumtest = LineTests(lambda line: re.search('^\d+|[xiv]+$',line['text'].strip()))
...
class MakeParagraphs(object):
    ...
    def __init__(self,pbrobj: Any,test:bool =True):
        self.pbrobj = pbrobj

    def proc_prosp(self):
        pages = self.pbrobj.pages
        last_lines,first_lines,pagelinear = [],[],[]
        for page in tqdm(pages):
            lines = page.extract_text_lines()
            if lines:
                lines = removepagenum(lines)
                last_lines.append(lines[-1])
                first_lines.append(lines[0])
                dists = np.array(self.get_dists(lines))
                ...
                pagelinear.append(siox.getvalue())
                siox.close()
        return pagelinear,first_lines,last_lines
